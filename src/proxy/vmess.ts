...
const ALLOWED_JSON_KEYS = ['v', 'ps', 'add', 'port', 'id', 'aid', 'scy', 'net', 'type', 'host', 'path', 'tls', 'sni', 'fp', 'pbk'] as const;
...
type RawVmessRecord = Record<string, unknown>;

const isPlainRecord = (value: unknown): value is RawVmessRecord => {
  return typeof value === 'object' && value !== null && !Array.isArray(value);
};

const decodeBase64Json = (encoded: string): RawVmessRecord => {
  if (encoded.length === 0) {
...
const getString = (record: RawVmessRecord, key: string, required = false): string | undefined => {
...
const assertAllowedKeys = (record: RawVmessRecord): void => {
  const allowedKeySet = new Set<string>(ALLOWED_JSON_KEYS);
...
const parseAlterId = (value: string | undefined): number => {
  if (value === undefined) {
    return 0;
  }
...
export const parseVmessUri = (nodeUrl: string): ParsedVmessShareLink => {
...
export const normalizeVmessUri = (nodeUrl: string, options: NormalizeProxyOptions): VmessProxy => {
  ...
  return {
    ...
    'reality-opts':
      parsed.realityPublicKey === undefined
        ? undefined
        : {
