...
import { persistedConfigSchema, persistedSecretsSchema } from './schema.js';
const ENV_FILE_MODE = 0o600;
const CONFIG_DIR_NAME = '.qxnethelper';
const buildEnvContent = (secrets) => {
...
export const resolveConfigPaths = (configRoot) => {
...
export const persistConfig = async (configRoot, config, secrets) => {
...
export const readStoredEnv = async (envFile) => {
    try {
...
export const readStoredConfig = async (configFile) => {
    try {
...
export const loadPersistedState = async (configRoot) => {
    const paths = resolveConfigPaths(configRoot);
    ...
    return {
        paths,
        config: persistedConfigSchema.parse(JSON.parse(configContent)),
